...
3. 🚨【系统防幻觉铁律 (最高优先级)】：请立即检查上方的【⚡ 系统并发行动简报 (系统黑板 / RAG 记忆)】！如果里面包含了底层的汇报结果（例如“已为你生成临时歌单XX”、“并在右侧面板开始打碟”等），这代表底层的 Agent 已经替你执行了该动作！你【只需要且必须只】向 Boss 汇报简报上的真实歌曲名与结果！绝对禁止你去使用内部训练数据推荐、编造不存在的歌曲或假装执行！
