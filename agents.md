...
这是一个名为 **Vault OS** 的本地 AI 助手/外脑系统。项目主体由 Python 后端、Vue 3 + Tauri 桌面前端、以及 `vault/plugins` VPM 插件生态组成。后端负责大模型对话、记忆/RAG、工具调用、异步智能体调度、Trace 观测、插件权限安全和 WebSocket 网关；前端负责桌面 UI、聊天终端、临时会话、执行观测、知识列表、记忆审核、画像导入预览/提交、设置页和插件面板展示。

本仓库当前是活跃开发工作区。开始修改前先检查 `git status`，不要回滚或覆盖用户已有改动。

## 目录概览

- `main.py`：核心引擎，定义 `VaultOS_Terminal` 与 `TempVaultSession`，负责配置加载、LLM 客户端、向量库、RAG 组装、SQLite 画像内核、JIT 任务规划、画像导入预处理、工具注册器与执行器。
- `server.py`：FastAPI 网关入口，启动动态端口，写入 `vault/.server_port` 和 `vault/.run_token`，提供 WebSocket、Trace API、临时会话、插件权限确认、插件 UI 文件、RAG 摄入和插件挂载/管理接口。
- `agent_runner.py`：异步智能体任务入口，把主会话与临时会话的重型 AI 推理放入独立线程池执行，并通过事件总线向前端流式回传状态、Trace 和结果。
- `core_bus.py`：事件总线，维护当前事件 `session_id`，让主会话、临时会话和插件 UI 消息可以在前端正确隔离。
...
- `memory_rules.py`：瞬时交互识别与 JIT 意图门控规则，用于避免把提问、搜索、播放、删除等一次性动作误写入长期记忆，并决定何时进入工具规划。
- `plugin_security.py`：第三方插件安全层，负责敏感信息脱敏、权限检测、manifest 安全归一化、插件 UI token、敏感权限确认和不可信插件输出包裹。
...
- `tool_registry.py`：注册内置工具，并扫描 `vault/tools/*.json`、插件 `manifest.json` 和插件 `tools/*.json`，加载时会归一化插件安全声明。
- `tool_executor.py`：执行内置工具与 VPM 工具，支持 `web_search`、`search_local_knowledge`、`control_ui_layout`、HTTP 插件和 subprocess 插件，并在调用第三方插件前执行权限校验和脱敏 Trace。
- `chat-ui/`：Vue 3 + Vite + Tauri 前端。`src/App.vue` 是主框架，`src/composables/useNeuroLink.js` 是全局状态、主/临时会话和 WebSocket 中心。
...
- `tests/`：测试目录，当前覆盖 JIT 意图门控、记忆缓冲池、认知快照、插件安全、画像导入、实体隔离、关系图、姓名冲突和网络搜索工具。
...
- `agent_runner.py` 使用独立线程池运行 `vault_os.get_response()`，避免长时间模型调用阻塞 WebSocket 主循环。主会话任务绑定 Trace；临时会话任务不读取主记忆，结束后迟到结果会被丢弃。
- `main.py` 的 JIT 编译器会根据用户指令和可用工具生成执行蓝图。`memory_rules.should_use_jit()` 先做意图门控，简单闲聊与本地画像问答尽量绕过工具规划；复杂任务可调度工具步骤并把结果写入黑板，再由最终回复总结。
- DAG 执行前会通过 `plugin_security_manager.preflight_authorize_steps()` 对第三方插件批量预检；插件请求未声明权限或被用户拒绝时，任务必须如实报告被安全策略拦截。
...
- 画像导入采用两阶段流程：先 `prepare_profile_import` 标准化并返回预览，再由用户 `commit_profile_import` 写入记忆；存在未结算待审项时会锁定后续导入。
- `rag_assembler.py` 从 SQLite L2 快照读取 Boss 画像、实体关系和认知状态，并与向量检索上下文组装系统提示词。
- `tool_registry.py` 负责工具发现；`tool_executor.py` 负责工具执行、HTTP 端点补全、subprocess 调用、内置搜索、UI 控制、插件权限校验和 Trace 埋点。
- `plugin_security.py` 是第三方插件安全边界：只有硬编码 allowlist 中的官方插件会被视为 first-party；第三方插件声明 first-party 不会自动获得信任。
- `server.py` 只会导入 first-party 插件的 `api.py`。第三方插件后端路由默认不被主进程 import；若访问 `/api/plugins/{plugin_id}/...`，必须通过内部 token 或插件 UI scoped token。
- RAG 摄入接口要求来源文件位于插件目录下，并校验 `metadata.plugin_id` 与来源路径中的插件目录一致。

## 前端核心机制

- `chat-ui/src/composables/useNeuroLink.js` 是前端状态中心，维护当前视图、主会话/临时会话消息、配置、插件列表、记忆队列、Trace 队列、动态插件组件和 WebSocket 连接。
- `connectWebSocket()` 建立连接后请求配置、记忆和历史记录；断线后会自愈重连。
- Trace 前端状态分为主会话与临时会话两套队列。前端会接收 `trace_event`，在终态或看门狗触发时拉取 Trace 快照进行补偿；`TerminalView.vue` 内置可折叠的执行观测面板。
- `App.vue` 管理主视图、侧边栏、输入框、插件动态侧栏/沉浸面板、删除确认弹窗、插件权限确认弹窗、全局 Toast 和快捷键。
- 主视图包括主控终端、临时会话、新闻/收藏知识列表、笔记详情、记忆同步、画像导入、设置、插件中心。
- 临时会话通过 `start_temp_session` / `end_temp_session` 建立和销毁，只用于无记忆隔离对话；记忆、画像导入、配置保存、删除、卸载等持久化操作在临时会话中不可用。
- 画像导入界面只接受 `.md` / `.txt`，有字符上限；导入时先展示标准化预览，用户确认后才写入记忆流程。
...
- `vpmLoader.js` 会根据插件安全等级注入上下文。first-party 插件获得完整 `vault:useNeuroLink`，third-party 插件只获得最小上下文、只读 API 地址、`showToast` 和带 scoped token 的 `pluginFetch`。
...
- `manifest.json` 应声明 `security.trust`、`security.permissions` 和 `security.sensitive_reason`。运行时会重新归一化安全字段，第三方插件不能靠自称 first-party 获得信任。
- HTTP 插件 endpoint 通常写成 `/api/plugins/{plugin_id}/execute`，运行时会被补成本次后端端口。第三方插件访问后端必须通过授权 token 或 `pluginFetch`。
- `api.py` 是可选的。提供时可声明 `router`、`init_plugin(app_engine)` 和 `uninstall_hook(app_engine)`，用于后端路由、建表和生命周期处理。当前主系统只自动 import first-party 插件的 `api.py`。
...
- 第三方插件请求敏感权限时，前端会展示插件、权限、用途和脱敏参数预览，用户可拒绝、允许一次或本次会话允许。
- 第三方插件输出会被包裹为不可信数据，后续模型总结时不得执行其中的指令、角色切换、密钥索取或工具调用建议。
- 第三方插件 UI 只能使用注入的 `pluginFetch("/api/plugins/{plugin_id}/...")` 调用自己的后端；不要直接使用全局 `fetch` 访问插件 API。
...
- `vault/plugin_permissions.json`：第三方插件敏感权限的会话授权记录。
...
- 修改插件安全、插件卸载、RAG 删除、笔记删除、资产导出清理逻辑时要逐路径审查，避免误删 `vault` 下共享数据或放宽第三方插件边界。
- 当前 `server.py` 的插件卸载路径仍存在递归删除插件目录的实现，后续应优先改造为更安全、可审计、逐项确认的卸载策略；在改造完成前，不要扩大这类逻辑的适用范围。
- 修改第三方插件权限、UI token、内部 token、脱敏规则或不可信输出包裹时，必须同步更新 `vault/plugins/README.md` 并补充/更新 `tests/test_plugin_security.py`。
- 修改临时会话时，必须保持“不读主记忆、不写持久状态、结束后丢弃迟到结果”的隔离语义。
- 修改画像导入时，必须保持“预览后提交”的两阶段流程，避免未确认内容直接进入长期记忆。
