...
- **Evaluation-driven iteration**：仓库已有测试与 Eval 测试矩阵，并已在 `docs/EVAL_REPORT.md` 记录首轮人工评测结果；可复核证据补档、指标趋势和反馈闭环仍是 TODO。
...
flowchart LR
    ...
    RESP --> APP
    TR --> APP
    EVAL["Eval / Manual Baseline (first results; no metrics trend yet)"] -.-> AR
    EVAL -.-> MEM
    EVAL -.-> TU
```
...
- **Eval**：单元测试与评测场景矩阵存在；`docs/EVAL_REPORT.md` 已归档首轮人工评测结果和证据缺口。它不是线上指标、自动化回归结论或长期质量趋势。路径：`tests/`、`docs/EVAL_REPORT.md`。
...
当前 Trace 记录根任务、DAG 步骤、记忆流程、LLM 最终生成、状态、耗时和错误，并带有 watchdog 超时处理。代码路径：`main.py`、`agent_runner.py`、`trace_system.py`。权限预检面向第三方插件的敏感权限；并非所有工具调用都会出现确认对话框。首轮人工评测还观察到某些 HTTP/网络工具失败未正确标记 Trace 状态，因此不能把现有 Trace 记录能力表述为所有失败路径均已可观察。
...
写入路径以减少记忆污染为目标：输入先经过瞬时交互识别与缓冲；候选事件再经路由、重复/冲突判断后进入合并或待审状态。画像导入也采用“准备预览 → 提交记忆流程”的两阶段语义，路径见 `main.py` 与 `chat-ui/src/views/ProfileImportView.vue`。首轮人工评测记录了虚构 PII 进入缓冲、待审、记忆库和原始对话日志，以及负面泛化内容进入画像的结果；这些是当前已识别风险，不应被上述设计目标掩盖。
...
- **评测边界**：`docs/EVAL_REPORT.md` 已记录首轮人工 RAG 结果，包括冲突来源未展示、RAG 注入执行和虚构 PII 直接披露等失败样例。历史结果的 Trace、截图和夹具编号尚未完整留存，且未形成召回率、答案质量指标或长期趋势；不得据此宣称 RAG 安全性或质量已被充分验证。
...
首轮人工评测确认了不可信插件输出包装和 manifest 自称第一方的阻断场景，同时记录了敏感参数在至少一条端到端调用路径中泄露、以及系统接受语义上过宽权限的风险。因此，代码中的脱敏和权限门控实现不能被表述为已完成端到端安全验证。

TODO：第三方插件后端 `api.py` 当前不会被自动挂载；其完整执行部署模型、插件可信来源体系、权限历史、语义化最小权限和数据删除语义仍需进一步定义。
...
当前可确认的是本地存储、会话隔离、插件权限确认和不可信插件输出包装；这不等同于完整安全体系。代码中包含参数脱敏逻辑，但首轮人工评测已记录敏感参数泄露的端到端风险，不能宣称脱敏已全面生效。Trace 会对部分文本和 payload 做脱敏，主运行时仍会把原始对话写入 `blackbox_raw.jsonl`（`main.py`），且评测记录了虚构 PII 进入该日志的情况，因此日志保留期限、访问控制和脱敏一致性是待解决的数据安全问题。
...
|---|---|---|
| RAG answer quality | `docs/EVAL_REPORT.md` 的首轮人工结果与证据缺口登记 | TODO：复跑并留存检索命中、来源展示、注入/PII 失败样例和人工质量判断。 |
| Memory write accuracy | `tests/test_memory_buffer_pool.py`、`tests/test_memory_cognitive_snapshots.py`、首轮人工结果 | TODO：补齐 PII/偏见风险证据，统计待审接受、拒绝与修正结果。 |
| Tool execution success | `tests/test_execution_health_plan.py`、`tests/test_web_search_tool.py`、首轮人工结果 | TODO：复跑 Trace 失败标记、敏感参数与任务取消场景，并聚合成功、失败、超时和降级比例。 |
| Plugin safety | `tests/test_plugin_security.py`、首轮人工结果 | TODO：补齐敏感参数泄露、语义化最小权限和卸载确认的审计样本。 |
| Temporary-session isolation | `docs/EVAL_REPORT.md` 的 TEMP 场景（首轮人工通过） | TODO：将隔离场景作为自动化回归并保存执行结果。 |

`docs/EVAL_REPORT.md` 已记录首轮人工评测：33 条通过、1 条部分通过、11 条未通过，另有证据缺口登记。该结果用于呈现当前能力与风险边界，不构成完整 Eval 报告、统计基线、线上指标或长期质量趋势；历史手工结果仍需按报告中的证据缺口复跑补档。
...
- TODO：补齐 RAG 的来源引用、检索反馈、摄入质量检查，并复跑首轮人工评测中的证据缺口。
