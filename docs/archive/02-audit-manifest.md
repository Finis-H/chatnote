...
| --- | --- | --- | --- |
| CLN-018 | 重建快照已记录解释器、pip、`requirements.txt` 哈希和脱敏依赖清单。 | 未执行删除。仓库规则禁止由 Codex 递归删除目录。 | 如仍要释放此路径容量，须由用户手动处理明确路径；之后再做存在性与空间复核。 |
| CLN-021 | 两个目录均命中临时 vault、`system_config.json`、`chat_history.json`、`vault_trace.db`/WAL/SHM 和日志等运行时数据线索；未读取文件内容。 | 已停止处理两个目录，未执行删除。 | 按 C-1 停止条件保留，等待用户完成隐私/保留决策；不得把它们当作普通构建缓存批量清理。 |
| CLN-022 | 当前 sidecar 仍存在；仓库内 NSIS 源候选哈希与 `VAULT-OS-PRIV-NSIS-001` 既有记录相符，但私有归档位置未写入仓库，无法仅凭标识再次访问确认。 | 未执行删除。 | 在用户提供可访问的私有归档核验路径并由用户手动处理明确目录前，保留整个 `target/`。 |

因此 C-1 没有完成任何删除，也没有改变 CLN-018、CLN-021、CLN-022 的原始保守风险判断；它只补充了已批准范围的重建快照与停止证据。
