...
| CLN-023 | `chat-ui/src-tauri/target/release/bundle/nsis/Vault OS_0.1.0_x64-setup.exe` | Windows 安装包 | 2026-07-16 NSIS 私有候选，78,230,654 bytes | 构建脚本输出位置；SHA-256 已记录，且与 `dist/` 副本一致 | 高 | 移出主仓库 | OQ-CLOSE-003/OQ-CLOSE-007 要求安装包保持私有；尚未安装验证、签名验证或公开。 | 先完成版本、哈希、安装/卸载与隐私审查，再移至受控私有位置并记录去向。 | 待确认 |
| CLN-024 | `dist/Vault OS_0.1.0_x64-setup.exe` | 重复安装包 | NSIS 候选副本 | 与 CLN-023 SHA-256 完全相同 | 中 | 删除 | 与保留的私有候选重复；无需保留两个工作树副本。 | 先完成 CLN-023 的私有归档，再逐文件删除此副本并复核哈希记录仍可定位。 | 待确认 |
| CLN-025 | `chat-ui/src-tauri/target/release/bundle/msi/Vault OS_0.1.0_x64_en-US.msi` | Windows 安装包 | 2026-05-20 MSI 候选，83,910,656 bytes | 当前 `tauri.conf.json` targets 仅为 `nsis`；未验证版本、来源或安装行为 | 高 | 待人工确认 | 可能是较早的交付证据或过期产物，不能仅按当前 target 删除。 | 先核对构建来源、版本、哈希、签名与私有归档需要。 | 待确认 |
| CLN-026 | `dist/vault_engine.exe` | 旧 Python sidecar | 2026-05-24 单文件私有候选，104,569,380 bytes | 当前 Tauri 配置使用 `bin/vault_engine` 目录式 sidecar；无路径引用该文件 | 高 | 移出主仓库 | 现有快照明确列为私有 sidecar 候选；未验证，不宜公开或直接丢弃。 | 先确认是否需作为历史交付证据，记录 SHA-256 后移入受控私有位置或另行批准删除。 | 待确认 |
