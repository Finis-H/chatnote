...
| CLN-018 | `.venv/`（30,402 文件，约 710.7 MB） | 本地依赖环境 | 本机构建/测试环境 | `.gitignore` 忽略；快照已记录 Python/pip 版本和 `requirements.txt` | 低 | 删除 | 可由清单重建，不是归档源码或证据链；保留清单与快照即可。 | C-1：用户手动删除后，路径复核为不存在；删除前记录的逻辑文件大小为 710,737,557 bytes。 | 已批准并完成（C-1；2026-07-17） |
...
| CLN-021 | `temp/windows-release-build-20260716/`、`temp/windows-release-onedir-20260716/`（约 569.7 MB 的 `temp/`） | 临时发行 staging | 2026-07-16 Windows 打包中间/目录式 sidecar 候选 | 构建脚本每次新建带时间戳 staging 目录；现有目录未被配置引用 | 中 | 停止处理 | C-1 文件名级预检命中临时 vault、`system_config.json`、聊天历史、SQLite Trace/WAL/SHM 与日志；未经内容读取不得将其视为普通缓存。 | 转入 OQ-DECOM-004，由用户决定隐私、保留、恢复与销毁条件；本轮保持两个目录不变。 | 停止处理：转入 OQ-DECOM-004（C-1；2026-07-17） |
| CLN-022 | `chat-ui/src-tauri/target/debug/`、`target/release/` 中非 `bundle/` 缓存（约 10.86 GB 的 `target/` 总量） | Rust/Tauri 编译缓存 | 本地 Cargo/Tauri 构建中间产物 | `src-tauri/.gitignore` 忽略；仅 Tauri 构建工具隐式使用 | 中 | 删除 | 可由 Cargo/Tauri 重建；安装包候选另列，不能连同 bundle 一并处理。 | C-1：用户手动确认 `VAULT-OS-PRIV-NSIS-001` 存在且哈希匹配后删除整个 `target/`；路径复核为不存在，删除前逻辑文件大小为 10,774,388,720 bytes。 | 已批准并完成（C-1；2026-07-17） |
| CLN-023 | `chat-ui/src-tauri/target/release/bundle/nsis/Vault OS_0.1.0_x64-setup.exe` | Windows 安装包 | 2026-07-16 NSIS 私有候选，78,230,654 bytes | 构建脚本输出位置；SHA-256 已记录，且与 `dist/` 副本一致 | 高 | 移出主仓库 | OQ-CLOSE-003/OQ-CLOSE-007 要求安装包保持私有；尚未安装验证、签名验证或公开。 | B-1 已完成私有归档；C-1 中用户再次确认 `VAULT-OS-PRIV-NSIS-001` 存在且哈希匹配，随后随 `target/` 一并手动删除仓库内源候选。未运行、安装、签名或公开。 | 私有归档已确认；仓库源候选已随 C-1 `target/` 删除（2026-07-17） |
...
用户在 2026-07-17 明确批准 C-1 范围：CLN-018（`.venv/`）、CLN-021（两个 Windows staging 目录）和 CLN-022（`chat-ui/src-tauri/target/`）。在任何删除前，已建立 `REBUILD_ENVIRONMENT.md` 并完成仅文件名级预检。

| 候选 | C-1 预检结果 | 执行状态 | 保留与后续 |
| --- | --- | --- | --- |
| CLN-018 | 重建快照已记录解释器、pip、`requirements.txt` 哈希和脱敏依赖清单。 | 用户手动删除；复核确认 `.venv/` 不存在。 | 已按快照中的命令保留环境重建基线；从删除前记录的逻辑文件大小中释放 710,737,557 bytes。 |
| CLN-021 | 两个目录均命中临时 vault、`system_config.json`、`chat_history.json`、`vault_trace.db`/WAL/SHM 和日志等运行时数据线索；未读取文件内容。 | 停止处理，两个目录仍存在。 | 转入 OQ-DECOM-004；在用户完成数据保留决策前不得按容量缓存删除。 |
| CLN-022 / CLN-023 | 当前 sidecar 与 `bin/_internal/` 仍存在；用户手动确认 `VAULT-OS-PRIV-NSIS-001` 私有归档存在且哈希匹配。 | 用户手动删除 `chat-ui/src-tauri/target/`；复核确认路径及其中的 CLN-023 仓库源候选均不存在。 | 从删除前记录的逻辑文件大小中释放 10,774,388,720 bytes；私有归档保留，当前 sidecar 保留。 |

因此 C-1 已完成其获准范围中的 CLN-018 与 CLN-022（并同步完成 CLN-023 仓库源候选的移出）；CLN-021 因运行时数据线索停止处理并转入 OQ-DECOM-004。两个手动删除路径在删除前记录的逻辑文件大小合计 11,485,126,277 bytes；未运行 Build、测试、服务、Docker、Tauri 或安装包。
